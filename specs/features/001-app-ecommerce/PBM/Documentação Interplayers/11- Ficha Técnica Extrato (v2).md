...
| extractSales[n].transaction[1].origin[n] | A | M | N/A | Origem da venda; |
...
| extractSales[n].terminalId | A | M | 1/10 | Identificação do terminal de origem das requisições, BALCAO, ECOMMERCE, CHECKOUT etc. |
...
| extractSales[n].product[30].discountValueUnit | N | M | 1/14 | Valor do desconto unitário que foi autorizado para o produto sem ser multiplicado pela quantidade; |
| extractSales[n].product[30].discountAdditionalMultiplied | N | M | 1/14 | Valor do desconto adicional multiplicado pela quantidade que foi autorizado para o Produto; |
...
| extractSales[n].product[30].statusDescription | A | M | N/A | Status do produto indicando aprovação do pedido |
...
| extractSales[n].retcode | A | O | 1/10 | Código único de retorno interno da plataforma, utilizado para apoio nas atividades de suporte. Atrelado a regra e domínio do negócio; |
| extractSales[n].informativeText | A | O | 1/1024 | Texto informativo referente ao código único de retorno; |
| Search[1].initialItem | N | M | 1/100 | Id da primeira transação localizada dentro da página. |
...
*Fim do documento transcrito (9 páginas).*
